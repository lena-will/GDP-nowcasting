...
# which_x <- category_choice %>%
#   filter(tau <= 0.1)
# 
# categ <- which_x$column
# x_pre <- as.data.frame(x)
# x_pre <- x_pre %>%
#   select(any_of(categ))
  ...
  x_ridge_p1 <- x_ridge[1:75, ]
  
  mean <- apply(x_ridge_p1, 2, mean)
  sd   <- apply(x_ridge_p1, 2, sd)
  x_ridge_p1_z <- scale(x_ridge_p1, center = mean, scale = sd)
  
  y_p1 <- as.data.frame(y[1:75, 1]) %>%
    rename(y = 1)
  y_p1 <- as.matrix(y_p1)
  ...
  for (ii in 1:length(alpha_ini)) {
    ident <- diag(ncol(x_ridge_p1_z))
    alpha <- alpha_ini[ii] * n
    beta_hat_pls <-
      solve(t(x_ridge_p1_z) %*% x_ridge_p1_z + alpha * ident) %*% t(x_ridge_p1_z) %*%
      y_p1
    y_hat_pls <- x_ridge_p1_z %*% beta_hat_pls
    gcv[ii] <-
      (1 / n) %*% t(y_p1 - y_hat_pls) %*% (y_p1 - y_hat_pls) / (1 -
                                                                  sum(diag(
                                                                    x_ridge_p1_z %*% solve(t(x_ridge_p1_z) %*% x_ridge_p1_z + alpha * ident) %*% t(x_ridge_p1_z)
                                                                  )) * (1 / n)) ^ 2
  }
  ...
  print(alpha_min)
  beta_hat_opt <-
    solve(t(x_ridge_p1_z) %*% x_ridge_p1_z + alpha_min * ident) %*% t(x_ridge_p1_z) %*%
    y_p1
  y_in_sample <- x_ridge_p1_z %*% beta_hat_pls
  
  mse <- mean((y_p1 - y_in_sample)^2)



tmp_y1 <- as.data.frame(y_p1)

df <- as.data.frame(cbind(y_in_sample, y_p1))
names <- c("y_in_sample", "y_p1")
colnames(df) <- names


plot <- ggplot(df, aes(seq(1,75, 1))) +
  geom_line(aes(y = y_in_sample, color = "black")) +
  geom_line(aes(y = y_p1, color = "red"))
