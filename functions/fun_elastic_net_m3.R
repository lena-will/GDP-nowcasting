    ...
    for (ii in 1:nrow(alpha_ini)) {
      for (month in 1:(nrow(window) - 2)) {
        y_m3_train <- y_m3 %>%
          ...
          select(gdp)
        y_m3_train <- as.matrix(y_m3_train)
        X_m3_train <- X_m3 %>%
          filter(Month >= min_train & Month <= window[month]) %>%
          select(-Month)
        
        mean_x <- apply(X_m3_train, 2, mean)
        sd_x <- apply(X_m3_train, 2, sd)
        
        X_m3_train_z <- scale(X_m3_train, center = mean_x, scale = sd_x)
        ...
        predict_en <-
          predict(fit_en, s = fit_en$lambda.1se, newx = X_m3_test_z)
